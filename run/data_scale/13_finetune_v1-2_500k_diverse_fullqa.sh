#!/bin/bash
#SBATCH --job-name=ds_ft_v1-2_500k_diverse_fullqa
...
echo "[ds_ft_v1-2_500k_diverse_fullqa] nodes=${SLURM_JOB_NODELIST}  master=${MASTER_ADDR}:${MASTER_PORT}"

unset SLURM_TRES_PER_TASK

for qa_dir in data/raw/finetune_qa_diversified \
              data/raw/finetune_qa_diversified_val; do
    if [[ ! -d "${qa_dir}" || -z "$(ls -A "${qa_dir}" 2>/dev/null)" ]]; then
        echo "[error] ${qa_dir} missing or empty; run python -m scripts.build_splits" >&2
        exit 3
    fi
done
...
echo "[ds_ft_v1-2_500k_diverse_fullqa] done"
