#!/bin/bash
#SBATCH --job-name=ds_ft_v1-2_200k_diverse
...
# ── data-scale 200k 变体: stage-2 ft only, 共享 v1-2_500k_diverse stage-1 ckpt ──
#   train_subsample_ratio=0.2104  →  ~200k QA train slots
#   Step budget: 130 steps/ep × 10 ep ≈ 1,300 steps → ~4-5h 实际, 12h limit.
# ────────────────────────────────────────────────────────────────────────────
...
CONFIG="experiments/training/configs/data_scale/v1-2_200k_diverse.yaml"
...
echo "[ds_ft_v1-2_200k_diverse] nodes=${SLURM_JOB_NODELIST}  master=${MASTER_ADDR}:${MASTER_PORT}"
...
echo "[ds_ft_v1-2_200k_diverse] done"
