"""
AO training: Classification + LatentQA, last-token, layer 75%.
Follows original nl_probes/sft.py settings (batch_size=16, lora r=64/alpha=128, lr=1e-5).

Launch (single GPU):
    torchrun --nproc_per_node=1 tests/ao/train_ao_cls_latentqa.py
"""
...
def eval_all_datasets(cfg, eval_datasets, model, tokenizer, submodule, device, dtype, global_step):
    model.eval()
    with torch.no_grad():
        cls_data = [dp for ds, data in eval_datasets.items() if ds != "latentqa" for dp in data]
        if cls_data:
            total, n = 0.0, 0
        ...
        if "latentqa" in eval_datasets:
            total, n = 0.0, 0
            for start in range(0, len(eval_datasets["latentqa"]), cfg.train_batch_size):
                batch_list = eval_datasets["latentqa"][start : start + cfg.train_batch_size]
                batch_list = materialize_missing_steering_vectors(batch_list, tokenizer, model)
                batch = construct_batch(batch_list, tokenizer, device)
                total += train_features_batch(cfg, batch, model, submodule, device, dtype).item()
                n += 1
            avg = total / n
            print(f"{EXP} [val step {global_step}] latentqa  | loss={avg:.4f}  ppl={math.exp(min(avg, 20)):.2f}", flush=True)
    model.train()
...
def build_datasets(cfg, dataset_loaders, latentqa_val_size=500, max_len_percentile=0.999):
    set_seed(cfg.seed)
    ...
    for loader in dataset_loaders:
        ...
        if loader.dataset_config.dataset_name == "latentqa":
    ...
    # hold out a slice of latentqa for val, rest goes to training
    if latentqa_train:
...
if __name__ == "__main__":
    ...
    all_loaders = cls_loaders + [latentqa_loader]
    ...
    cfg = SelfInterpTrainingConfig(
        ...
        save_steps=5000,
        save_dir="../tests/ao/experiments/ao_cls_latentqa/checkpoints",
    )
